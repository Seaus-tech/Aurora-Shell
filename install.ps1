# v5.8.0
# Aurora-Shell v5.8.0 installer — PowerShell port
...
function Generate-Theme {
    ...
    & $a '}'
    & $a ''
    & $a '# shell command — use shell.aurora -mc to install the jacket-based module system'
    & $a '# shell install|search|list|update|outdated <package>'
    & $a ''
    & $a ''
    ...
    & $a '    if(-not $jacket){ Write-Host "No jacket for $p ($key) — falling back" -ForegroundColor Yellow; _fallback $p; return }'
    ...
    & $a '    $binSrc = "$dest\$($jacket.bin).exe"'
    & $a '    if(-not(Test-Path $binSrc)){$binSrc="$dest\$($jacket.bin)"}'
    & $a '    Copy-Item $binSrc "$SHELL_DIR\bin\" -Force -ErrorAction SilentlyContinue'
    ...
    & $a '    "list"      { Get-ChildItem $CELLAR_DIR -ErrorAction SilentlyContinue | ForEach-Object { "$($_.Name) $(Get-ChildItem $_.FullName | Select-Object -First 1 -ExpandProperty Name)" } }'
    ...
    & $a '    default     { Write-Host "Usage: shell install|uninstall|list|search|update|outdated <package>"; Write-Host "Jackets: github.com/Seaus-tech/Aurora-Shell/.pack/shell/jackets" }'
    & $a '}'
    & $a '''' 
    ...
    & $a '        }'
    & $a '        default { Write-Host "Flags: --display --sys --update --config --lock --uninstall --account --motd --doctor --history --run --sync --modules-components (-mc)" }'
    & $a '    }'
    & $a '}'
    & $a '# MOTD'

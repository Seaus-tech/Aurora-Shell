...
function Generate-Theme {
    ...
    & $a '        "--motd"      { $m = try { $r = irm "https://zenquotes.io/api/today" -TimeoutSec 5; "$($r[0].q) — $($r[0].a)" } catch { $null }; if ($m) { if (Get-Command lolcat -ErrorAction SilentlyContinue) { $m | lolcat } else { Write-Host $m -ForegroundColor Cyan } } else { Write-Host "No MOTD available." } }'
    & $a '        "--doctor"    {'
    & $a '            Write-Host "Aurora-Shell Doctor" -ForegroundColor Cyan'
    & $a '            $ok = $true'
    & $a '            if ($env:PATH -notlike "*aurora-shell_files\bin*") { Write-Host "WARN: aurora-shell_files\bin not in PATH" -ForegroundColor Yellow; $ok=$false }'
    & $a '            if (Test-Path $PROFILE) { if (-not (Select-String "aurora-shell_theme" $PROFILE -Quiet)) { Write-Host "WARN: Theme not sourced in profile" -ForegroundColor Yellow; $ok=$false } }'
    & $a '            foreach ($cmd in @("git","node","npm","figlet","lolcat")) { if (Get-Command $cmd -ErrorAction SilentlyContinue) { Write-Host "OK  $cmd" -ForegroundColor Green } else { Write-Host "MISSING $cmd" -ForegroundColor Red; $ok=$false } }'
    & $a '            if ($ok) { Write-Host "All checks passed" -ForegroundColor Green }'
    & $a '        }'
    & $a '        "--history"   {'
    & $a '            $h = Get-Content (Get-PSReadLineOption).HistorySavePath -ErrorAction SilentlyContinue'
    ...
    & $a '        }'
    & $a '        default       { Write-Host "Flags: --display --sys --update [branch] --config --lock --uninstall --account --motd --doctor --history --run --sync" }'
    & $a '    }'
    & $a '}'
    & $a ''
